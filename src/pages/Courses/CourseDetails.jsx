...
const CourseDetails = () => {
  ...
  const fetchCourseData = async () => {
    try {
      ...
      if (user && isStudent()) {
        const progressRes = await progressAPI.getByStudentAndCourse(
          user.id,
          parseInt(id)
        );
  ...
  const handleEnroll = async () => {
    ...
    try {
      ...
      const nextProgressId =
        Math.max(...allProgress.data.map((p) => p.id), 0) + 1;
  ...
  if (loading) {
    return (
      <div className="container loading" style={{ paddingTop: "2rem" }}>
        ...
        <div className="course-header-content">
          ...
          <div className="course-info">
            ...
            {progress && (
              <span className="course-info-item">
                ...
                {progress.progressPercentage.toFixed(0)}% Complete
              </span>
            )}
          </div>
          ...
          {user && isStudent() && (
            <div className="course-actions">
              {enrolled ? (
                <Link
                  to={`/courses/${id}/lessons/${lessons[0]?.id || 1}`}
